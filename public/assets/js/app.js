...
jQuery(document).ready(function($) {
    ...
    if( ! hasFlash ){
        ...
        if(!is_safari){
            jQuery('#start-streaming').attr('disabled', true).addClass('disabled');
        }
    }
});
...
$('#reset-submit').click(function(){
       ...
       conpassword = $('#conpassword').val().trim();
       if(password=='' || conpassword=='')
        ...
        $.ajax({
                 ...
                 success: function(msg) {
                     if(msg == 'success'){
                         alertify
                        .alert("Your pasword has been changed.", function(){
       ...
       if(email=='')
        ...
        $.ajax({
                 ...
                 success: function(msg) {
                  //   console.log(msg);
                     if(msg == 'invalid'){
                         alertify.alert("We don't have any account with this email address.");
                     }
                     else if(msg == 'done'){
                         $('#email-forget').val('');
                         alertify.alert("We have sent you an email, please check your inbox to reset your password.");
                     }
...
$('#privacy-save').click(function(){
       ...
       conpassword = $('#confirm-password').val().trim();
                     ...
                     if(msg == 'false'){
                         ...
                         $.ajax({
                                ...
                                success: function(msg){
                                    ...
                                    if(msg=='success'){
                                        $.loader('close');
                                        alertify.alert('Privacy settings have been successfully changed.');  
...
$('#register-submit').click(function(){
           email = $('#email').val().trim();
           username = $('#username').val().trim();          
           login_type = $('#login_type').val().trim();
        ...
        function unfollowuser(follower_id,following_id){
            ...
            $.ajax({
                ...
                success: function(msg){
                    var but = "<img id='loader' style='display: none' src='"+ baseurl +"/assets/images/gloader.gif' alt='spinner'> <button type='button' class='followerbutton' onClick=\"followuser('"+follower_id+"','"+following_id+"')\">Follow</button>";
        ...
        function followuser(follower_id,following_id){
            ...
            $.ajax({
                ...
                success: function(msg){
                    var but = "<img id='loader' style='display: none' src='"+ baseurl +"/assets/images/gloader.gif' alt='spinner'> <button type='button' class='followerbutton' onClick=\"unfollowuser('"+follower_id+"','"+following_id+"')\">UnFollow</button>";
                    $('.follow-buttons-'+following_id).html(but);
                    $('#loader').hide();
                }
            });
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            for(i=0;i<data.length;i++){
                                html += '<a href="'+baseurl+'main/view_broadcast/'+data[i].id+'" class="search_by_name"> '+
                                    '<img src="'+data[i].broadcast_image+'">'+
                                    '<strong>'+data[i].title+'</strong> </a>';
                            }
...
if(type == "Broadcast"){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            var html='';
            for(i=0;i<data.length;i++){
                html += '<a href="'+baseurl+'profile/'+data[i].username+'" class="search_by_name"> '+
                    '<img src="'+data[i].profile_picture+'">'+
                    '<strong>'+data[i].username+'</strong> </a>';
            }
            $('.search_data').html(html);
            $('div.search_data').addClass('show_result');
            ;                }
...
//                }
        ...
        function postSuccess(data, textStatus, jqXHR) {
         ...
         function displayComment(data) {
            ...
            com = $('.comment-count-'+data.broadcast_id).html().split(' ');
        ...
        function createComment(data) {        
          ...
          var html = ''+
                  '<li><figure><a href="javscripti:void(0)"><img src="'+data.comment.profile_picture+'" alt="#"></a></figure>'+
                    '<div class="text">'+
                       '<h2><a href="javscripti:void(0)">'+data.comment.username+'</a><time>'+data.comment.date+'</time></h2>'+
                        '<p>'+data.comment.comment.replace(new RegExp("\\\\", "g"), "")+'</p>'+
                    '</div>'+
                  '</li>';
        ...
        function like_broadcast(user_id,broadcast_id){
            $.ajax({
                ...
                success: function(data){
                    data = $('.likes-'+broadcast_id).html().split(' ');
                    likes = parseInt(data[0])+1;
                    $('.likes-'+broadcast_id).html(likes+' likes');
                    var html = '<img src="'+baseurl+'assets/images/icon-unlike.png" onClick="unlike_broadcast('+user_id+','+broadcast_id+')">';
        ...
        function unlike_broadcast(user_id,broadcast_id){
            $.ajax({
                ...
                success: function(data){
                    ...
                    likes = parseInt(data[0])-1;
                    $('.likes-'+broadcast_id).html(likes+' likes');
                    var html = '<img src="'+baseurl+'assets/images/icon-like1.png" onClick="like_broadcast('+user_id+','+broadcast_id+')">';
                    $('.like-button-'+broadcast_id).html(html);
