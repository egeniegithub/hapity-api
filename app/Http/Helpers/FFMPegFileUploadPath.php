...
function ffmpeg_upload_file_path($source_file_path){
    ...
    if(file_exists($source_file_path)) {
        ...
        Log::channel('ffmpeg_logs')->info('shell_exec(cp '.$shell_exec);
        ...
        Log::channel('ffmpeg_logs')->info('shell_exec(ffprobe -loglevel error -select_streams v:0 -show_entries stream_tags=rotate -of default=nw=1:nk=1 '.$shell_exec);

        if($shell_exec == 90){
            unlink($source_file_path);
            $shell_exec = shell_exec('ffmpeg -i "'.$temp_pathtosave.'" -vf "transpose=1,transpose=2" '. $source_file_path);
            
            Log::channel('ffmpeg_logs')->info('shell_exec(ffmpeg -i '.$shell_exec);

            $shell_exec = shell_exec('rm '.$temp_pathtosave);

            Log::channel('ffmpeg_logs')->info('shell_exec(rm '.$shell_exec);
